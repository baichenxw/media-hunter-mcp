...
def create_server(config: Config | None = None, service: MediaService | None = None) -> FastMCP:
    ...
    async def get_post(site: str, post_id: str, ctx: Context) -> ToolResult:
        """作品完整元数据。post_id 为数字；ehentai 为 gid/token。"""
        return await call("get_post", ctx, site=site, post_id=post_id)
    ...
    async def download_url(
        ...
        """
        return await call(
            "download_url", ctx, url=url, subdir=subdir, timeout=timeout, overwrite=overwrite
        )

    @app.tool(title="检查站点连通性", annotations=READ_ANNOTATIONS, output_schema=OUTPUT_SCHEMA)
    async def self_check(ctx: Context) -> ToolResult:
        """并行检查四站凭证及 API 连通性，每站最多 45 秒；不下载媒体、不返回凭证。"""
        return await call("self_check", ctx)
